...
//       // If categoryName is empty, call a general "get all published skills" endpoint,
//       // else call "search by category"
//       const url = categoryName
//         ? `${
//             import.meta.env.VITE_BASE_URL
//           }/admin/skills/searchname/${categoryName}`
...
const ExploreList = () => {
  ...
  useEffect(() => {
    const fetchPublishedSkills = async () => {
      setIsLoading(true);
      setError("");
      try {
        const accessToken = localStorage.getItem("accessToken");
        ...
        setError(
          err.message || "Unable to load skills. Please try again later."
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const EmptyState = () => (
    <div className="flex flex-col items-center justify-center py-16 text-gray-500">
      ...
      <p className="text-center max-w-md text-secondary text-[12px]">
        No skills are currently published. Check back later.
      </p>
    </div>
  );
    ...
    <UserLayout>
      <div className="max-w-4xl mx-auto px-4 rounded-lg">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {skills.map(({ _id, title, description, thumbnail }) => (
            <div
              key={_id}
